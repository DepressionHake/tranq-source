...
public:
	...
	void LogPlayer(IClientEntity* Entity, CUserCmd* cmd)
	{
		if (!Entity || !cmd) // nullptr checks
			return;
		if (Entity->GetTeamNum() == hackManager.localplayer()->GetTeamNum())
			return;
		...
		if (fabs(deltaTime) > 0.2f)
		{
			...
			Utilities::Log("Lagcomp: delta too big (%.3f)\n", deltaTime);
	...
	void BacktrackPlayer(IClientEntity* Entity, CUserCmd* cmd, float flTargetTime)
	{
		...
		Vector delta = CurPlayerData[Entity->index()].m_vecOrigin - prevOrg;
		...
		prevOrg = CurPlayerData[Entity->index()].m_vecOrigin;
		float frac = 0.0f;
		if (CurPlayerData &&
			(CurPlayerData[Entity->index()].m_flSimulationTime < flTargetTime) &&
			(CurPlayerData[Entity->index()].m_flSimulationTime < PrevPlayerData[Entity->index()].m_flSimulationTime))
		{
			// we didn't find the exact time but have a valid previous record
			// so interpolate between these two records;

			Assert(PrevPlayerData[Entity->index()].m_flSimulationTime > CurPlayerData[Entity->index()].m_flSimulationTime);
			Assert(flTargetTime < PrevPlayerData[Entity->index()].m_flSimulationTime);

			// calc fraction between both records
			frac = (flTargetTime - CurPlayerData[Entity->index()].m_flSimulationTime) /
				(PrevPlayerData[Entity->index()].m_flSimulationTime - CurPlayerData[Entity->index()].m_flSimulationTime);

			Assert(frac > 0 && frac < 1); // should never extrapolate

			ang = Lerp(frac, CurPlayerData[Entity->index()].m_vecAngles, PrevPlayerData[Entity->index()].m_vecAngles);
			org = Lerp(frac, CurPlayerData[Entity->index()].m_vecOrigin, PrevPlayerData[Entity->index()].m_vecOrigin);
			minsPreScaled = Lerp(frac, CurPlayerData[Entity->index()].m_vecMinsPreScaled, PrevPlayerData[Entity->index()].m_vecMinsPreScaled);
			maxsPreScaled = Lerp(frac, CurPlayerData[Entity->index()].m_vecMaxsPreScaled, PrevPlayerData[Entity->index()].m_vecMaxsPreScaled);
		}
		else
		{
			// we found the exact record or no other record to interpolate with
			// just copy these values since they are the best we have
			org = CurPlayerData[Entity->index()].m_vecOrigin;
			ang = CurPlayerData[Entity->index()].m_vecAngles;
			minsPreScaled = CurPlayerData[Entity->index()].m_vecMinsPreScaled;
			maxsPreScaled = CurPlayerData[Entity->index()].m_vecMaxsPreScaled;
		}
		LagRecord* restore = &RestoreData[Entity->index()];
		LagRecord* change = &ChangedData[Entity->index()];
	...
	{
		int iTarindex;
		if (iTarindex == -1 || iTarindex > interfaces::engine->GetMaxClients())
			return false;

		if (!CurPlayerData[iTarindex].m_bValidRecord)
			return false;

		// forgot lerptime storage for oldest record but fuck it w.e
		pCmd->tick_count = TIME_TO_TICKS(CurPlayerData[iTarindex].m_flSimulationTime);
		return true;
	};
	...
	void CLagCompensation::log(ClientFrameStage_t stage)
	{
		...
		if (g_menu.rage.AccuracyBacktracking.getstate())
		{
			if (stage == ClientFrameStage_t::FRAME_NET_UPDATE_END)
			{
				...
				{
					IClientEntity *player = reinterpret_cast<IClientEntity*>(player = interfaces::g_entitylist->GetClientEntity(i));
