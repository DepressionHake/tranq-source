...
void grenade_prediction::View(CViewSetup* setup)
{
	...
	{
		CBaseCombatWeapon* weapon = (CBaseCombatWeapon*)interfaces::g_entitylist->GetClientEntityFromHandle(local->GetActiveWeaponHandle());
...
void grenade_prediction::Setup(Vector& vecSrc, Vector& vecThrow, Vector viewangles)
{
	...
	vecThrow = local->velocity(); vecThrow *= 1.25f;
