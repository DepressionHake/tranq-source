...
void SkinChanger() {
    IClientEntity *localplayer = EntList->GetClientEntity(Engine->GetLocalPlayer());
    auto weapons = localplayer->m_hMyWeapons();
    for (size_t i = 0; weapons[i] != INVALID_EHANDLE_INDEX; i++)
    {
		IClientEntity *m_entity = EntList->GetClientEntityFromHandle(weapons[i]);
        if (m_entity)
        {
            CBaseCombatWeapon* pWeapon = (CBaseCombatWeapon*)m_entity;
            if (g_menu.AntiAimTab.fakeyaw.getindex())
            {
                int Model = g_menu.AntiAimTab.fakeyaw.getindex;
                int weapon = *pWeapon->m_AttributeManager()->m_Item()->ItemDefinitionIndex();
                switch (weapon)
                {
                case 7: // AK47 
                {
                    switch (g_menu.AntiAimTab.fakeyaw.getindex)
                    {
                    case 0:
                ...
                if (m_entity->GetClientClass()->m_ClassID == (int)ClassID::CKnife)
                {
                    ...
                    if (Model == 0) // Bayonet
                    {
                        ...
                        *pWeapon->GetEntityQuality() = 3;
                        ...
                        int Skin = g_menu.AntiAimTab.fakeyaw.getindex;
                        if (Skin == 0)
                        {
